...
void hdp_hyperparameter::setup_parameters(double _gamma_a, double _gamma_b, double _alpha_a,
...
void doc_state::setup_state_from_doc(const document * doc) {
	...
	if (m_doc_length > 0){
		...
		m_is_valid = false;
		printf("Invalid document found at line = %d\n", m_doc_id+1);
...
//	double total_q = update_q_f(d_state, w, q, f);
