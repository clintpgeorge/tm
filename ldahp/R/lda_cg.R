lda_cg <- function(K, V, wid, doc.N, alpha.v, eta, max.iter=100, burn.in=0, spacing=1)
{
  ...
  ret          <- .Call("lda_cg", K, V, doc.N, wid-1, zid-1, alpha.v, eta, max.iter, burn.in, spacing, PACKAGE="ldahp");
  
  list(z=ret$z+1, theta=ret$ts, beta=ret$bs, lmp=ret$lmp);
