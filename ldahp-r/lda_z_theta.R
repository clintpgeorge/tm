lda_z_theta <- function(K, V, ds, alpha.v, eta, max.iter=100, burn.in=0, spacing=1)
{
  ...
  for (iter in 1:max.iter) {      
    ...
    for (d in 1:D) {
      ...
      beta <- normalize(beta.counts + eta, dim=2);            
    ...
    if ((iter > burn.in) && (iter %% spacing == 0)) {
      ...
      betas[,,count] <- normalize(beta.counts + eta, dim=2); # saves beta (This is not a wise idea for a large vocabulary)
      count <- count + 1;
    }
